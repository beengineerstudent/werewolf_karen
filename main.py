    ...
    while True:
        ...
        if scene == SCENE_TITLE:
            ...
            if player_index < len(players) - 2:
                ...
                most_suspicious_player = max(players, key=lambda p: p[
                    'villager_poll_count'])
            ...
            if player_index_for_poll < len(players) - 2:
                ...
                poll = sorted(players[:-2], key=lambda p: int(p[
                    'poll_count']), reverse=True)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if scene == SCENE_TITLE:
                    ...
                    scene, villager_poll_index, player_index, players = (await
                    ...
                    scene = await update_fortune_teller_select_scene()
                elif scene == SCENE_FORTUNE_TELLER:
                    scene, fortune_tell_index, is_grave = (await
                        update_fortune_teller_scene(fortune_tell_index=
                        fortune_tell_index, fortune_tell_index_max=players.
                        index(players[-1]) - 2, is_grave=is_grave))
                elif scene == SCENE_FORTUNE_TELLER_ROLE:
